...
DATABASE_FILE_NAME = "manao.db"
SQLALCHEMY_DATABASE_URL = (
    f"sqlite+aiosqlite:///{DATABASE_FILE_PATH}{DATABASE_FILE_NAME}"
)
...
async_session = sessionmaker(
    engine, autocommit=False, class_=AsyncSession  # type: ignore
)  # type: ignore
    ...
    async def find_by_id(cls, db_session: AsyncSession, id: str):
        ...
        if not instance:
            raise HTTPException(
                ...
                detail=f"Record of {cls.__name__} not found ({id=})",
            )
        return instance
